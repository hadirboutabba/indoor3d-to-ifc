...
This document describes the full pipeline from raw scanner output to IFC model.
...
└────────────────────┬────────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────────┐
│  SPATIALLM INFERENCE  (inference.py)        │
...
│  • Collinear wall detection & merging       │
│  • Bounding box refitting to point cloud   │
└────────────────────┬────────────────────────┘
                     │ refined layout .txt
                     ▼
┌─────────────────────────────────────────────┐
...
└─────────────────────────────────────────────┘
                     ...
                     ▼
┌─────────────────────────────────────────────┐
│  IFC BUILDER  (spatiallm_to_ifc.py)         │
│  • Parse layout entities                    │
│  • Map to IFC4 schema                       │
│  • Write .ifc file via IfcOpenShell         │
└────────────────────┬────────────────────────┘
                     │ .ifc
                     ▼
...
- `position_{x,y,z}` is the door centre in world coordinates
- IFC: `IfcDoor` placed as a solid aligned to the host wall direction
- **Note:** Wall openings (`IfcOpeningElement` / `IfcRelVoidsElement`) are not yet implemented; door solids are placed inside walls without cutting holes.
...
- IFC: `IfcWindow` placed as a solid aligned to the host wall direction
- **Note:** Same limitation as doors — no wall cutout geometry.
...
Generated by `spatiallm_to_ifc.py` using [IfcOpenShell](https://ifcopenshell.org/).
  ...
  └─ IfcSite  "Site"
       └─ IfcBuilding  "Building"
            └─ IfcBuildingStorey  "Ground Floor"
                 ├─ IfcSlab              (floor — convex hull footprint, 10 cm thick)
                 ├─ IfcWall × N
                 ├─ IfcDoor × N         (solid, not a wall opening — see limitations)
                 ├─ IfcWindow × N        (solid, not a wall opening — see limitations)
                 └─ IfcFurnishingElement × N  (labelled with furniture category)
```

All elements use `IfcExtrudedAreaSolid` geometry and `IfcLocalPlacement` for positioning.
Each element carries a unique GUID generated at conversion time.
